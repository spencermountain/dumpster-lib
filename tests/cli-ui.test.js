...
test('CLI flag run prints setup and results as plain ASCII, with heartbeat disabled', async () => {
  ...
  assert.match(stdout, /Results\s+done/)
  assert.match(stdout, /processed\s+20/)
  assert.match(stdout, /throughput/)
  assert.doesNotMatch(stdout, /\x1b|[^\x00-\x7f]/)
  assert.doesNotMatch(stdout, /overall/)
  assert.equal(stderr, '')
})

test('CLI silent run emits nothing and invalid options fail quietly', async () => {
  const { stdout, stderr } = await exec(process.execPath, [bin, fixture.file, '--workers', '1', '--silent'], { env })
  assert.equal(stdout + stderr, '')
  await assert.rejects(exec(process.execPath, [bin, '/missing.xml', '--silent'], { env }), (err) => {
    assert.equal(err.code, 1)
    assert.equal(err.stdout + err.stderr, '')
    return true
  })
})

test('non-terminal guided and missing-option invocations fail without hanging', async () => {
  for (const args of [[], ['-i'], ['--format', 'text']]) {
    await assert.rejects(exec(process.execPath, [bin, ...args], { env, timeout: 5000 }), (err) => {
      assert.equal(err.code, 1)
      assert.match(err.stderr, /requires a terminal|missing required/)
      return true
    })
  }
})

test('prompt coercion preserves number defaults, parses extensions, and validates', () => {
  assert.equal(promptValue({ type: 'number' }, '', 12), 12)
  assert.equal(promptValue({ type: 'number' }, '4', 12), 4)
  assert.throws(() => promptValue({ type: 'number' }, 'wrong'), /number/)
  assert.equal(promptValue({ type: 'string', parse: (value) => value.toUpperCase() }, 'en'), 'EN')
  assert.throws(() => promptValue({ type: 'path', validate: () => 'missing file' }, 'nope'), /missing file/)
})
...
test('wizard keeps defaults and plugin choice types across text/select/boolean/number prompts', { timeout: 5000 }, async () => {
  ...
  assert.deepEqual(await result, { name: 'hello', choice: 9, enabled: true, count: 12 })
  assert.match(io.output(), /starting the run/)
})

test('wizard displays validation errors and allows correction', { timeout: 5000 }, async () => {
  const io = terminal()
  ...
  await delay(60)
  io.stdin.write('\r')
  assert.deepEqual(await result, { count: 3 }, io.output())
  assert.match(io.output(), /must be a number/)
})

test('wizard cancellation rejects and releases the terminal', { timeout: 5000 }, async () => {
  for (const key of ['\x03', '\x1b']) {
    const io = terminal()
    const result = promptParams([{ name: 'file', desc: 'File', type: 'path' }], {}, {}, io)
    const rejected = assert.rejects(result, PromptCancelled)
    await delay(60)
    io.stdin.write(key)
    await rejected
    assert.match(io.output(), /cancelled/)
  }
})

test('dashboard preserves worker states, counts, overall progress and queue statistics', () => {
...
test('writer extensions still receive parsed options and await batch/end listeners', async () => {
  ...
  assert.deepEqual(JSON.parse(stdout), { out: 'pages', written: fixture.expect.articles.length })
  assert.equal(stderr, '')
  await assert.rejects(exec(process.execPath, [writer, fixture.file, '--out', 'pages', '--fail'], { env, timeout: 5000 }), (err) => {
    assert.equal(err.code, 1)
    assert.equal(err.stdout + err.stderr, '')
    return true
  })
...
test('skip multi-select groups filters, preserves defaults, and continues to plugin prompts', { timeout: 5000 }, async () => {
  const io = terminal()
  ...
  assert.match(io.output(), /Step 1\/2/)
  assert.match(io.output(), /Which pages should be skipped/)
  assert.doesNotMatch(io.output(), /Y\/n|y\/N/)
})

test('skip multi-select toggles multiple filters and allows clearing all selections', { timeout: 5000 }, async () => {
  const filters = baseParams.filter((param) => param.name.startsWith('skip_'))
  const cases = [
