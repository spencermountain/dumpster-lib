...
test('every article arrives exactly once, across workers and partial batches', async (t) => {
...
test('a slow async writer is awaited, never overlapped, and bounds the queue', async (t) => {
...
test('a sync writer works with the same listener', async (t) => {
...
test('honours skip_disambig option', async (t) => {
...
test('flags stubs and honours skip_stub option', async (t) => {
...
test('a rejecting writer aborts the run, fires error, and rejects done', async (t) => {
  ...
  await rejects(t, pool.done, /db is down/)
  t.equal(errored.message, 'db is down')
  t.ok(calls < 10, 'stopped handing out batches after the failure')
})

test('end listeners are awaited before done resolves', async (t) => {
  const pool = dumpster({ ...base, workers: 2 })
  let closed = false
  pool.on('batch', () => {})
  pool.on('end', async (stats) => {
    t.ok(stats.written > 0)
    await sleep(30)
    closed = true
  })
  await pool.done
...
test('listeners attached after the call are still in place when the first batch lands', async (t) => {
...
test('md format includes templates', async (t) => {
  ...
  await pool.done
  t.equal(page.templates.find((t) => t.template === 'tinytown data').founded, '1801')
  t.ok(Array.isArray(page.links))
})

test('a missing file rejects done, with a helpful error', async (t) => {
  const pool = dumpster({ ...base, file: '/nope/enwiki-latest-pages-articles.xml' })
  await rejects(t, pool.done, /can't find file/)
})

test('an unknown format rejects done', async (t) => {
  const pool = dumpster({ ...base, format: 'yaml' })
  await rejects(t, pool.done, /unknown format 'yaml'/)
})

test('an invalid NSFW reason map rejects done', async (t) => {
  const pool = dumpster({ ...base, skip_nsfw: { Weapons: 'yes' } })
  await rejects(t, pool.done, /'skip_nsfw' must be true, false, or an object/)
})

test('an invalid skip_stub option rejects done', async (t) => {
  const pool = dumpster({ ...base, skip_stub: 'yes' })
  await rejects(t, pool.done, /'skip_stub' must be true or false/)
})

test('an invalid namespace rule rejects done', async (t) => {
  const pool = dumpster({ ...base, namespace: { 0: 'yes' } })
  await rejects(t, pool.done, /'namespace' must be an integer, boolean, null, or an object/)
})

test('an invalid silent option rejects done', async (t) => {
  const pool = dumpster({ ...base, silent: 'yes' })
  await rejects(t, pool.done, /'silent' must be true or false/)
})

test('renamed filter options fail instead of being silently ignored', async (t) => {
  const renamed = {
  ...
  for (const [oldName, newName] of Object.entries(renamed)) {
    ...
    await rejects(t, pool.done, new RegExp(`'${oldName}' has been renamed to '${newName}'`))
  }
})
